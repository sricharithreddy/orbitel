...
def initial_lead_funnel(df: pd.DataFrame):
    ...
    counts = pd.concat([pd.DataFrame({"Lead Status":["Unique Leads"],"Count":[total],"% of Unique Leads":["100%"]}),counts])
    return counts
...
def lost_lead_breakdown(df: pd.DataFrame):
    ...
    lead_reason = lost_leads.groupby("mobile_number")["Lost Reason"].agg(lambda s: s.mode().iat[0] if not s.mode().empty else "Other")
...
def duration_analysis(df: pd.DataFrame):
    return df[df["outcome_norm"].isin(["converted","lost"])].groupby("outcome_norm")["duration"].mean().round(2).reset_index().rename(columns={"outcome_norm":"Outcome","duration":"Avg Duration (sec)"})
